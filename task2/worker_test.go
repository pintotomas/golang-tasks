...
// TestWorker_Process this test creates a worker which will send a total of 5 requests to a limiter with 3 reqs per second, so only 3 should success
func TestWorker_Process(t *testing.T) {
	...
	expectedThrottle := int32(2) // Two requests should be throttled
	expectedSuccess := int32(3)  // Three requests should succeed
